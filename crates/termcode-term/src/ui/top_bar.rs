...
/// Width of the " ^Q Exit " button (including padding). Named after the key
/// that always quits, which no keymap can take away.
pub const EXIT_BUTTON_TEXT: &str = " ^Q Exit ";
pub const EXIT_BUTTON_WIDTH: u16 = 9;

/// The Settings button, sitting just left of Exit. ASCII only: a gear glyph is
...
pub struct TopBarButtons {
    pub exit_start: u16,
    /// `None` when the bar is too narrow to fit Settings as well; Exit is the
    /// one that stays, since a user who cannot type the quit key has no other
    /// way out.
...
impl TopBarButtons {
    ...
    pub fn left_edge(&self) -> u16 {
        self.settings_start.unwrap_or(self.exit_start)
    }
}

pub fn buttons(area: Rect) -> TopBarButtons {
    let exit_start = (area.x + area.width).saturating_sub(EXIT_BUTTON_WIDTH);
...
impl Widget for TopBarWidget<'_> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        draw_button(buttons.exit_start, EXIT_BUTTON_TEXT);
    }
}
...
mod tests {
    ...
    #[test]
    fn top_bar_keeps_exit_button() {
        let row = render_row(80);
        assert!(row.ends_with(EXIT_BUTTON_TEXT), "got: {row:?}");
    }

    #[test]
    fn top_bar_title_does_not_overrun_exit_button() {
    ...
    #[test]
    fn top_bar_puts_settings_left_of_exit() {
        let row = render_row(80);
        let expected = format!("{SETTINGS_BUTTON_TEXT}{EXIT_BUTTON_TEXT}");
        assert!(row.ends_with(&expected), "got: {row:?}");
    }

    #[test]
    fn a_narrow_bar_keeps_exit_and_drops_settings() {
    ...
    fn the_title_stops_before_the_leftmost_button() {
        // 32 columns: both buttons fit (22), leaving 10 for the title, which is
        // shorter than " termcode vX.Y.Z" -- so it must be cut, not overlap.
        let row = render_row(32);
        let buttons = super::buttons(Rect::new(0, 0, 32, 1));
        ...
        assert!(
            !title.contains("Settings") && !title.contains("Exit"),
            "got: {title:?}"
        );
    }
    ...
    fn button_positions_stay_inside_the_bar() {
        for width in [1, 8, 9, 20, 21, 22, 40, 200] {
            ...
            assert!(buttons.exit_start < width.max(1), "width {width}");
            if let Some(start) = buttons.settings_start {
                assert!(start < buttons.exit_start, "width {width}");
